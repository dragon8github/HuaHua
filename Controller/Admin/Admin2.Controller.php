...
class Admin2Ctrl
{
    ...
    public function get_所有信息()
    {
        ...
        IF(@$orderby == "yonghuyue") //用户余额
        {
            $DESC = "A.balance";
        }
        ELSE  IF(@$orderby == "yonghutixianzonge") //提现总额
        {
            $DESC = "mysum";
        }
        ELSE  IF(@$orderby == "chuticishu") //出题总数
        {
            $DESC = "myquestion_cont";
        }
        ELSE  IF(@$orderby == "daticishu") //答题次数
        {
            $DESC = "myanswer_count";
        } 
    ...
    public function get_获取用户充值的总额($openid)
    {
        //选择表
        $this->Sql->table = 'statements';
        //条件语句
