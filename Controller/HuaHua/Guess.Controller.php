...
class GuessCtrl
{
    ...
    public function get_headpic()
    {
        ...
        //发送语句
        $arr = $this->Sql->where($where)->field('wx_litpic')->select();
        //返回结果
        return $arr;
    }
    ...
    public function Ajax_流水记录和微信支付json($stype,$money,$openid,$cot)
    {
		...
		$prict_count=$this->C_金额转换($money)*$cot;
        //选择表
        $this->Sql->table = 'statements';
        //数据结构
        $data["id"] = $orderid;
        ...
        $data2["prop"] =$this->C_金额转换($money)*0.5;
 ...
 public function get_tips()
   ...
   public function get_tip_rows()
   {
    ...
    public function Ajax_购买道具($order,$money,$uid)
    {
        //选择流水表
        $this->Sql->table = 'statements';
        //条件语句
    ...
    public function Ajax_微信支付json()
    {
        ...
        $q = $_GET["q"];
        //自定义sql
        $mysql = sprintf("SELECT uid,(prop) AS daoju,B.tips,B.tips2 FROM question AS A JOIN question_library AS B ON A.answer = B.id WHERE A.id =  '%s'",$q);
        //发送语句，获得道具金额
        ...
        $tips = $daoju[0]["tips"];                  //答案1
        $tips2 = $daoju[0]["tips2"];              //答案2
        ...
        $arr = array('Msg' => '请求成功！' , 'Result' => array('order' => $orderid,'tips' =>$tips,'tips2' =>$tips2 ,'money' => $daojujiage, 'uid' =>$uid ,'wxjson' => $jsApiParameters) , 'Status' => '成功' );
        //返回为json
        exit(json_encode($arr));
    }
...
IF(@$_POST["type"] == 'GouMaiDaoJu')
{
    ...
    $money = $_POST["money"];
    $uid = $_POST["uid"];
    $_GuessCtrl->Ajax_购买道具($order,$money,$uid);
}
