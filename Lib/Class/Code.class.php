 ...
 * 		//实例化对象，可传两个参数，若不传则使用默认值
 * 		$code = new Code([int 字体大小,int 验证码位数]);
 * 		//输出验证码图片
 * 		$code->getImage();
 * 		//校验，正确返回True , 错误返回False
 * 		Code::check(string 验证码,[int 过期时间]);
 * 		注：验证码MD5加密后保存在$_SESSION['vcode'] 创建时间 $_SESSION['vtime']
 * 		
 * 可选前端参数[GET]：
 * 		codeFont  设置字体 1-5 超出范围则自动使用默认值
 * 		codeSize  字体大小，用于控制验证码图片尺寸
 * 		例：http://loaclhost/code.php?codeFont=1&codeSize=20   //设置1号字体，大小为20px
 * 		注：参数优先顺序  前端 => 后端  => 默认
 */
class Code{
	...
	public $type = 2;			//验证码类型 0.纯数字  1.纯字母  2.混合  (设定了自定义字符集时无效)
